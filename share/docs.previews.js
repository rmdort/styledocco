...
var htmlEntitiesReverse = function(str) {
...
iframeEl.addEventListener('load', function() {
  ...
  forEach(bodyEl.getElementsByClassName('preview-code'), function(codeEl, idx) {
    addIframe(codeEl, support, idx);
    resizeableButtons();
    autoResizeTextArea(codeEl);
  });

  var selectCodes = document.getElementsByClassName('selectCode');

  forEach(selectCodes, function(selectCode){
    selectCode.addEventListener('click', function(event){
...
var addIframe = function(codeEl, support, iframeId) {
  ...
  var selectCode = document.createElement('a');
  selectCode.className = "selectCode";
  selectCode.href = "#";
  selectCode.textContent  = "Select Code";
  previewEl.appendChild(selectCode);
  ...
  iframeEl.addEventListener('load', function() {
    ...
    // Otherwise replace iframe content with the preview code.
      ...
      bodyEl.innerHTML = htmlEntitiesReverse(codeEl.nextSibling.innerHTML);
  ...
  if (!support.sameOriginDataUri) {
    ...
    iframeSrc = 'data:text/html;charset=utf-8,' +
      encodeURIComponent('<!doctype html><html><head></head></body>' +
        htmlEntitiesReverse(codeEl.innerHTML));
  }
...
var resizeableButtons = function() {
  ...
  var resizeableElOffset = 40; // `.resizeable` padding
